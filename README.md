...
Lee et al. (2016). OrthoANI: An improved algorithm and software for calculating average nucleotide identity. *International Journal of Systematic and Evolutionary Microbiology*.  
DOI: 10.1099/ijsem.0.000760

In contrast to the original Java-based GUI OAT software described by Lee et al. (2016), this implementation enables automated batch processing of multiple genome FASTA files within a single directory.

---

## Background

This tool was originally developed during my PhD (2020) after encountering Java/JavaFX compatibility issues while using the Java-based GUI OAT (OrthoANI Tool) on macOS (Intel).
...
Unlike the original GUI-based OAT implementations (e.g., EZBioCloud, OrthoANI), this version supports unlimited genome comparisons and is designed for Unix/Linux server and HPC environments.

---

## Validation

ANI values generated by this implementation are consistent with those produced by the original OAT software when identical genome inputs and BLAST parameters are used. Minor numerical differences may occur due to BLAST version or system-specific variations.
...
- Command-line based (Unix/macOS/Linux compatible)  
...
Lee et al. (2016). OrthoANI: An improved algorithm and software for calculating average nucleotide identity. *Int J Syst Evol Microbiol*. DOI: 10.1099/ijsem.0.000760
